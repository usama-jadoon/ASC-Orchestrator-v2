"""Validate the documentation that defines the shipped PESE runtime contract."""
...
def main() -> int:
    """Return nonzero when a canonical PESE or CLI documentation invariant is absent."""

    readme = README.read_text(encoding="utf-8")
    pese = PESE.read_text(encoding="utf-8")
    errors: list[str] = []

    for command in ("state", "resume", "checkpoint", "validate-state"):
