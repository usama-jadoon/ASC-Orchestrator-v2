...
No active implementation work. M016 AGC agent lifecycle and the registration → release lifecycle is complete; encrypted transport and autonomous workflow scheduling remain outside the current release scope.
...
M016 implements the canonical AGC v1.0 agent-lifecycle contract on top of the PESE/TBE/MSS/EEF/CKS/AEX/AHP/VAL/RKM foundations, closing the lifecycle gap: the agent ledger gives M017's recovery engine a deterministic view of which agents are READY, BUSY, BLOCKED, FAILED, or QUARANTINED, and gives M019's autonomous scheduler the eligibility signal it needs for unattended dispatch. AGC operates agent records over PESE `agent_state`, enforces the agent status state machine through `AGENT_STATUS` transitions, requires `VERIFIED` dependency environments before READY, enforces orchestrator/self actor authority, and emits `AGENT_*` events to the EEF execution journal. The release is complete for intake, team-assembly, persistent-state, execution-lifecycle, cryptographic-signing, agent-execution, agent-health, validation, risk-management, and agent-lifecycle scope; encrypted transport and autonomous workflow scheduling remain intentionally absent.
