...
class TestUniversalIdentityAndState(unittest.TestCase):
    ...
    def test_scenario_a_composite_task_identity_no_collision(self):
        ...
        self.state.update_task_status("T1", TaskStatus.COMPLETED, mission_id="mission-alpha")
    ...
    def test_scenario_b_duplicate_mission_id_safety(self):
        ...
        self.state.update_task_status("T1", TaskStatus.FAILED, mission_id="mission-repeat")
    ...
    def test_scenario_g_attempt_budget_survives_restart(self):
        ...
        try:
            attempts = state2.get_attempts("T1", mission_id="mission-budget")
            # Next increment should be 3
            next_attempt = state2.increment_attempt_count("T1", mission_id="mission-budget")
        ...
        spec = MissionSpec(
            ...
            tasks=[
                Task(
                    ...
                    commands=[
                        VerificationCommand(command="pytest tests/foo_test.py", timeout=120),
    ...
    def test_scenario_p_recent_events_recency_order(self):
        ...
        self.assertEqual(event_types, ["EVENT_16", "EVENT_17", "EVENT_18", "EVENT_19", "EVENT_20"])
...
""", encoding="utf-8")
        ...
        driver = MissionDriver(spec=spec, working_directory=str(self.repo_dir), adapter=MockAdapter())
        self.assertEqual(Path(driver.working_directory).resolve(), self.repo_dir.resolve())
...
class TestGitSafetyAndInterruption(unittest.TestCase):
    ...
    def setUp(self):
        ...
        subprocess.run(["git", "add", "."], cwd=self.repo_dir, check=True)
        subprocess.run(["git", "commit", "-m", "init commit"], cwd=self.repo_dir, check=True)
    ...
    def test_scenario_f_interrupted_running_task_reconciliation(self):
        ...
        state.record_attempt(task_id="T1", attempt_number=1, status=TaskStatus.RUNNING, exit_code=0, mission_id="crash-mission")
        state.close()
    ...
    def test_scenario_i_and_j_dirty_repo_preflight_blocks_run_and_resume(self):
        ...
        (self.repo_dir / "dirty_untracked.txt").write_text("dirty content", encoding="utf-8")
    ...
    def test_scenario_k_unrelated_midrun_change_not_staged(self):
        ...
        spec = MissionSpec(
            ...
            tasks=[
                Task(id="T1", title="Task 1", prompt="Create file", commit_paths=["task_output.txt"])
            ],
        )
        driver = MissionDriver(spec=spec, working_directory=str(self.repo_dir), adapter=SideEffectAdapter())
        res = driver.run()
        self.assertEqual(res["final_status"], "COMPLETE")
        ...
        head_show = subprocess.run(["git", "show", "--name-only", "--oneline", "HEAD"], cwd=self.repo_dir, capture_output=True, text=True).stdout
    ...
    def test_scenario_l_failed_attempt_rollback_preserves_unrelated_directories(self):
        ...
        subprocess.run(["git", "add", "."], cwd=self.repo_dir, check=True)
        subprocess.run(["git", "commit", "-m", "add user docs"], cwd=self.repo_dir, check=True)
...
class TestMultiCommandVerificationAndProcesses(unittest.TestCase):
    ...
    def test_scenario_n_large_output_stream_draining_no_deadlock(self):
        ...
        try:
            ...
            fake_task = Task(id="T_LARGE", title="Large stream test", prompt="some prompt")

            with patch.object(adapter, "_get_omp_executable", return_value=str(fake_omp_bat)):
                res = adapter.execute(fake_task, {
    ...
    def test_scenario_o_timeout_process_tree_termination(self):
        ...
        try:
            ...
            hanging_bat.write_text(f'@"{sys.executable}" "{hanging_py}" %*\n', encoding="utf-8")

            fake_task = Task(id="T_HANG", title="Hang test", prompt="hang", execution_timeout=1)

            with patch.object(adapter, "_get_omp_executable", return_value=str(hanging_bat)):
                start = time.time()
                res = adapter.execute(fake_task, {
                    "working_directory": temp_dir,
                    "execution_timeout": 1,
