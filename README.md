...
- REL v1.0 production-release verifier — a deterministic, stdlib-only release gate that certifies packaging metadata (version 1.0.2, dependency-free wheel contract, `src/` layout), canonical contract coverage, runtime module importability, and per-contract test suite presence, with a machine-readable `release` command output
...
- [RELEASE_NOTES.md](./RELEASE_NOTES.md) — v1.0.2 maintenance release announcement and quick start
